...
_lift1  = createVehicle ["Sign_Sphere10cm_F",getpos player,[], 0, ""];
_lift1  setPos [_v select 0,_v select 1,_v select 2];
_lift1 setDir (_o-180);
...
player attachTo [_lift1, [0,-0.6,_d]];
...
while {keepClimb} do { // add check for height and check that player cant press up and down at the same time creating a infinite loop.
        ...
        if (climbUp) then {
                ...
                if ((getPosATL player select 2) >= (_f - 2)) then {
                        player attachTo [_lift1, [0,-1.1,_d]];
